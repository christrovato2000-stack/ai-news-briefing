...
Usage:
  python main.py                  # run the full briefing pipeline
  python main.py --dry-run        # fetch + summarize but don't send email
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate and send an AI & Tech news briefing."
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Fetch and summarize news but do not send the email.",
    )
    parser.add_argument(
        "--days",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to save the rendered HTML email (e.g. output.html).",
    )
    parser.add_argument(
        "--save-json",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Claude model to use for summarization (default: claude-opus-4-6).",
    )
...
def main() -> int:
    ...
    logger.info("=" * 60)
    logger.info("AI & Tech News Briefing starting")
    logger.info("  Look-back: %d days | Dry run: %s", args.days, args.dry_run)
    logger.info("=" * 60)

    # ── Step 1: Aggregate news ────────────────────────────────────────────────
    logger.info("STEP 1/3 — Aggregating news…")
    try:
    ...
    logger.info("STEP 2/3 — Summarizing with Claude (%s)…", args.model)
    try:
    ...
    # Count categorized stories
    total_categorized = sum(
        len(v) for v in briefing.get("categories", {}).values()
    )
    logger.info(
    ...
    if args.dry_run:
        logger.info("STEP 3/3 — DRY RUN: skipping email send.")
        print("\n" + "=" * 60)
        ...
        try:
            send_briefing(briefing, output_path=args.output)
        except Exception as exc:
            logger.error("Fatal error during email send: %s", exc, exc_info=True)
            return 1

    # Optionally save HTML even when not dry-run
    if args.output and not args.dry_run:
        logger.info("HTML also saved to %s", args.output)
